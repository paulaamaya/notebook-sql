...
ORDER BY dir.first_name FETCH FIRST 10 ROW ONLY;

SELECT  dir.first_name
       ,dir.last_name
       ,mov.movie_name
       ,mov.release_date
FROM DIRECTORS AS dir
INNER JOIN MOVIES AS mov
ON dir.director_id = mov.director_id
WHERE mov.movie_lang IN ('Chinese', 'Korean', 'Japanese')
...
FROM MOVIES AS mov
INNER JOIN MOVIE_REVENUES AS rev
ON mov.movie_id = rev.movie_id
WHERE movie_lang = 'English'
ORDER BY rev.international_takings;
...
INNER JOIN MOVIE_REVENUES AS rev
ON mov.movie_id = rev.movie_id
WHERE rev.domestic_takings IS NULL OR rev.international_takings IS NULL
...
INNER JOIN MOVIES AS mov
ON ma.movie_id = mov.movie_id
WHERE mov.movie_lang = 'English';

-- OPERATORS
SELECT  first_name
       ,last_name
FROM DIRECTORS UNION
SELECT  first_name
       ,last_name
FROM ACTORS
ORDER BY first_name;